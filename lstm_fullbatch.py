...
def full_batch_grad(model, X, y, criterion, batch_size=128):
    """Tính gradient full-batch bằng cách accumulate gradient qua các mini-batch."""
    model.train()
    model.zero_grad()
    total = 0
    for i in range(0, len(X), batch_size):
        xb = X[i:i+batch_size].to(DEVICE)
        yb = y[i:i+batch_size].to(DEVICE)
        loss = criterion(model(xb), yb) * len(xb) / len(y)
        loss.backward()
        total += loss.item()
    return total
...
# ── Adam (full-batch) ────────────────────────────────────
print("=" * 60)
print("ADAM OPTIMIZER (full-batch)")
print("=" * 60)
...
for epoch in range(EPOCHS+20):
    tl = full_batch_grad(model_adam, X_small, y_small, criterion)
    optimizer.step()
    optimizer.zero_grad()
    ...
    print(f"  Epoch {epoch+1:>2}: train_loss={tl:.4f} | val_loss={val_loss:.4f} | val_acc={val_acc*100:.2f}%")
...
#
# Full-batch: gradient tính trên toàn bộ tập train mỗi epoch.

print("\n" + "=" * 60)
print("QUICKPROP OPTIMIZER (full-batch)")
print("=" * 60)
...
for epoch in range(EPOCHS):
    tl = full_batch_grad(model_qp, X_small, y_small, criterion_qp)
    qp_optim.step()
    qp_optim.zero_grad()
...
print("-" * 62)
print(f"{'Adam (full-batch)':<24} {best_loss_adam:>10.4f} {best_acc_adam*100:>9.2f}% {best_epoch_adam:>6} {t_adam:>7.1f}s")
print(f"{'QuickProp (full-batch)':<24} {best_loss_qp:>10.4f} {best_acc_qp*100:>9.2f}% {best_epoch_qp:>6} {t_qp:>7.1f}s")
print("=" * 62)